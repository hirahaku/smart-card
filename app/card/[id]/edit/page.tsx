    ...
    <div className="min-h-screen bg-neutral-950 text-white pb-20">
      ...
      <main className="p-6 max-w-md mx-auto">
        <section className="mb-8">
          ...
          <InputField label="氏名" value={profile.full_name} field="full_name" />
          <InputField label="役職・肩書き" value={profile.job_title} field="job_title" />
        </section>

        <section className="mb-8">
          <h2 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">連絡先・SNS</h2>
          <InputField label="電話番号" value={profile.phone} field="phone" placeholder="09012345678" />
          <InputField label="メールアドレス" value={profile.email} field="email" placeholder="example@mail.com" />
          <InputField label="LINE ID" value={profile.line_id} field="line_id" placeholder="IDのみ入力" />
          <InputField label="Instagram ID" value={profile.instagram_id} field="instagram_id" placeholder="ユーザー名のみ" />
          <InputField label="X (Twitter) ID" value={profile.x_id} field="x_id" placeholder="ユーザー名のみ" />
          <InputField label="WebサイトURL" value={profile.website_url} field="website_url" placeholder="https://..." />
        </section>
